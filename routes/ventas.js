...
router.post('/',
  [
    ...
    // }).optional(),
    // check('valorUnitario', 'El valor unitario es requerida debe ser numérico').notEmpty().isNumeric(),
    // check('cantidad', 'La cantidad es requerida debe ser numérico').notEmpty().isNumeric(),
    // check('valorTotal', 'El valor total es requerida debe ser numérico').notEmpty().isNumeric(),
    validarCampos,
    // validarJWT
  ],
...
router.put('/:id',
  [
    // check('id', 'Se necesita un mongoId válido').isMongoId(),
    // check('fecha', 'La fecha debe estar en formato válido').optional().isISO8601().toDate(),
    // check('codigoProducto').custom(helpersVentas.postPutCodigoPto).optional(),
